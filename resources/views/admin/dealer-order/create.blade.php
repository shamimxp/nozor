...
<div class="content-header mb-4">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h2 class="page-title mb-1">Create Dealer Order</h2>
            <div class="d-flex align-items-center">
                <span class="text-muted mr-2">Reference ID:</span>
                <span class="ref-badge">{{ $orderNumber }}</span>
            </div>
        </div>
        <div>
...
<form action="{{ route('admin.dealer-order.store') }}" method="POST" enctype="multipart/form-data" id="dealerOrderForm">
    @csrf
    ...
    <div class="row">
        <div class="col-lg-8">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-4 form-group">
                            ...
                            <input type="text" name="order_number" class="form-control" value="{{ $orderNumber }}" readonly style="background: #f1f5f9 !important; font-weight: 700; color: #475569 !important; cursor: not-allowed;">
                        </div>
                        <div class="col-md-4 form-group">
                        ...
                        <div class="col-md-12 form-group mt-2 mb-0">
                            ...
                            <select name="dealer_id" class="form-control select2" required>
                                ...
                                @foreach($dealers as $c)
                                    <option value="{{ $c->id }}">{{ $c->shop_name }} ({{ $c->phone }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
            ...
            <!-- Design Images (Hidden by default) -->
        ...
        <div class="col-lg-4">
            <div class="summary-panel">
                ...
                <div class="summary-row border-0 pb-0">
                    <span>Discount</span>
                    <input type="number" name="discount" id="discount" class="form-control text-right" placeholder="0.00">
                </div>
                <div class="summary-row mt-2">
                    <span>Carrying Charge</span>
                    <input type="number" name="carrying_charge" id="carryingCharge" class="form-control text-right" placeholder="0.00">
                </div>
                ...
                <div class="mt-4 p-3 rounded" style="background: rgba(0,0,0,0.2);">
                    <div class="form-group mb-2">
                        <label class="text-uppercase text-muted" style="font-size: 0.75rem; letter-spacing: 1px; color: #94a3b8 !important;">Payment Received</label>
                ...
                <button type="submit" class="btn btn-submit btn-block mt-4 w-100">
                    <i data-feather="check-circle" class="mr-2"></i> CONFIRM ORDER
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        // Initialize Select2 - with search for large lists like dealers/products
    ...
    $(document).on('click', '.remove-item', function() {
        ...
        if (cart.filter(i => i.image && i.image !== '').length === 0) {
            $('#imagePreviewContainer').html('<div class="col-12 text-center text-muted py-3 img-empty-state">No images available.</div>');
        }
        renderCart();
    });
